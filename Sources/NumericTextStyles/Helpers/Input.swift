...
@usableFromInline struct ToggleSignInput {
    ...
    @inlinable init?<P: Parser>(consumable: inout Snapshot, parser: P) where P.Output == Sign {
        guard let sign = parser.parse(characters: consumable.characters) else { return nil }
